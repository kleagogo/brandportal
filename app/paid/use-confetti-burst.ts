/**
 * Confetti with real physics, from transitions.dev.
 *
 * Flakes fall through the stage and collide with the trigger element, which is
 * modelled as a pill: they rest on the flat top and slide off the curved caps.
 * Once everything settles the pile holds, then fades. Skipped entirely under
 * reduced motion — a celebration nobody asked for shouldn't override that.
 */

import { useCallback, useEffect, useRef, type RefObject } from 'react'

const COLORS = ['#ff4d67', '#ffb020', '#3b82f6', '#22c55e', '#a855f7', '#f97316', '#06b6d4', '#f43f5e']
...
export function useConfettiBurst(
  ...
  const burst = useCallback(() => {
    ...
    type Rect = ReturnType<typeof buttonRect>
    const buttonSurface = (x: number, b: Rect) => {
    ...
    const frame = (now2: number) => {
      if (!s.running) return
      // Substep so the physics tracks wall-clock even when rAF throttles.
