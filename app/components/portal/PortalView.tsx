...
export function PortalView({ config, portal }: { config: BrandConfig; portal: SharePortal }) {
  ...
  // White-label retinting: the client's accent overrides the theme's primary
  // for this portal only, scoped to the wrapper below rather than the document.
  const style = accent
    ? ({ '--primary': accent, '--primary-foreground': readableOn(accent) } as React.CSSProperties)
    : undefined
  ...
  return (
    <div className="flex items-center gap-3 min-w-0">
      {logo ? (
        ...
        <div className={`${box} bg-foreground rounded-xl shrink-0`} />
      )}
      <div className="min-w-0">
        <p className={`${size === 'sm' ? 'text-[13px]' : 'text-[15px]'} font-semibold leading-tight truncate`}>{name}</p>
        <p className="text-[11px] text-muted-foreground/60 leading-tight truncate">{branding.note || config.tagline}</p>
      </div>
    </div>
  )
}
      ...
      <header className="bg-card border-b border-border sticky top-0 z-20">
        <div className="max-w-4xl mx-auto px-5 sm:px-8 py-3 flex items-center gap-5">
          ...
          <nav className="ml-auto hidden sm:flex items-center gap-4 overflow-x-auto">
            {sections.map(s => (
              <a key={s.id} href={`#${s.id}`} className="text-[12.5px] text-muted-foreground hover:text-foreground transition-colors whitespace-nowrap">
                {s.label}
              </a>
            ))}
          </nav>
...
function MinimalTemplate({ config, portal }: { config: BrandConfig; portal: SharePortal }) {
  ...
  return (
    <div className="flex-1 max-w-2xl w-full mx-auto px-5 sm:px-8 py-14">
      ...
      {sections.length === 0 && (
        <p className="text-[13px] text-muted-foreground/60">No files have been shared here yet.</p>
      )}
        ...
        return (
          <div key={section.id} className="mb-10">
            <div className="flex items-center justify-between gap-4 mb-3">
              ...
              {allowDownload && hasLocalFiles && (
                <a
                  ...
                  className="text-[12px] font-semibold text-foreground underline underline-offset-2 whitespace-nowrap"
                >
                  Download all
                </a>
              )}
            ...
            <div className="border border-border rounded-2xl overflow-hidden bg-card">
              {assets.map((asset, i) => (
                <div key={`${asset.name}-${i}`} className="flex items-center gap-3 px-4 py-3 border-b last:border-b-0 border-border">
                  ...
                  {allowDownload && (
                    <a
                      ...
                      className="text-[12px] font-semibold px-3 py-1.5 rounded-lg bg-primary text-primary-foreground hover:opacity-85 transition-colors whitespace-nowrap shrink-0"
                    >
