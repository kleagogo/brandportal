...
/* transitions.dev Pro — smoky-dissolve. Vendored from transitions/smoky-dissolve/react.md.
   Typed for this codebase; behaviour unchanged. */

import { useCallback, useRef } from "react";

export function useSmokyDissolve(stageRef, cardRef, canvasRef, opts) {
  ...
  return useCallback(() => {
    if (!ctrl.current) {
      ...
      // Created lazily on first delete so the snapshot sees final layout.
      ctrl.current = createSmokyDissolve({ stage, card, canvas, ...opts });
    }
    ctrl.current.dissolve();
  }, []);
}

function DeletableImage({ src, onDeleted }) {
  const stageRef = useRef(null);
  const cardRef = useRef(null);
  const canvasRef = useRef(null);
  const dissolve = useSmokyDissolve(stageRef, cardRef, canvasRef, {
    respawn: false,
    onComplete: onDeleted,
  });

  return (
    <div className="t-smoky-stage" ref={stageRef}>
      <div className="t-smoky-card" ref={cardRef}>
        <img alt="" src={src} />
        <button
          type="button"
          className="t-smoky-delete"
      ...
      <canvas className="t-smoky-canvas" ref={canvasRef} aria-hidden />
    </div>
  );
}
...
export function createSmokyDissolve({ stage, card, canvas, respawn = true, onComplete }) {
