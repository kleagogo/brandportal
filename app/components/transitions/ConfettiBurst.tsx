...
/* transitions.dev Pro — confetti-burst. Vendored from transitions/confetti-burst/react.md.
...
export function useConfettiBurst(stageRef, canvasRef, btnRef) {
  const state = useRef({
    particles: [],
    running: false,
    lastT: 0,
  ...
  const burst = useCallback(() => {
    ...
    const frame = (now2) => {
      ...
      if (settled && s.fadeStart === null)
        s.fadeStart = now2 + readNum("--confetti-hold", 1600);
      let alpha = 1;
...
function CelebrateCard() {
  ...
  return (
    <div className="t-confetti-stage" ref={stageRef}>
      ...
      <button type="button" ref={btnRef} onClick={burst}>
        Celebrate
      </button>
    </div>
  );
