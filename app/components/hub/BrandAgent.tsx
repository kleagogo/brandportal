...
export function BrandAgent() {
  ...
  return (
    <>
      <button
        onClick={() => setOpen(o => !o)}
        className="fixed bottom-6 right-6 w-12 h-12 bg-primary text-primary-foreground rounded-full flex items-center justify-center shadow-lg hover:opacity-85 transition-colors z-50"
      ...
      {open && (
        <div className="fixed bottom-20 right-6 w-80 max-w-[calc(100vw-3rem)] bg-card border border-border rounded-2xl shadow-xl z-50 flex flex-col overflow-hidden" style={{ height: 420 }}>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3">
            {messages.length === 0 && (
              <p className="text-[12px] text-muted-foreground leading-relaxed">{config.agent.greeting}</p>
            )}
            {messages.map((m, i) => (
              ...
              <div className="flex justify-start">
                <div className="bg-muted px-3 py-2 rounded-xl">
                  <div className="flex gap-1">
                    {[0, 1, 2].map(i => (
                      <div key={i} className="w-1.5 h-1.5 bg-muted-foreground/60 rounded-full animate-bounce" style={{ animationDelay: `${i * 0.15}s` }} />
                    ))}
                  </div>
                </div>
              </div>
          ...
          <div className="p-3 border-t border-border flex gap-2">
            <input
              ...
              className="flex-1 text-[13px] px-3 py-2 bg-muted rounded-lg outline-none placeholder:text-muted-foreground/60"
            />
            <button
              onClick={send}
              disabled={loading || !input.trim()}
              className="w-8 h-8 bg-primary text-primary-foreground rounded-lg flex items-center justify-center disabled:opacity-40 hover:opacity-85 transition-colors shrink-0"
            >
