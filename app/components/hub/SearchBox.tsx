    ...
    <div className="relative hidden md:block w-56">
      <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-muted border border-transparent focus-within:border-foreground focus-within:bg-card transition-colors">
        ...
        <input
          ...
          className="flex-1 min-w-0 bg-transparent text-[12.5px] outline-none placeholder:text-muted-foreground/60"
        />
      </div>

      {open && q.trim().length >= 2 && (
        <div className="absolute top-10 left-0 right-0 z-40 bg-card border border-border rounded-xl shadow-xl overflow-hidden">
          {results.length === 0 ? (
            <p className="px-3 py-2.5 text-[12px] text-muted-foreground/60">Nothing matches “{q.trim()}”</p>
          ) : (
            results.map((hit, i) => (
              <button
              ...
              >
                {hit.swatchHex ? (
                  <span className="w-4 h-4 rounded border border-foreground/10 shrink-0" style={{ background: hit.swatchHex }} />
                ) : (
                ...
                <span className="min-w-0">
                  <span className="block text-[12.5px] font-medium text-foreground truncate">{hit.label}</span>
                  <span className="block text-[11px] text-muted-foreground/60 truncate">{hit.detail}</span>
                </span>
              </button>
            ))
          )}
